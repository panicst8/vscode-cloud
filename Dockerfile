...
# Added this line because it hangs doing a new build when it asks interactive questions
...
# updating to tar because file is no longer zip, but tar.gz
# RUN unzip VSCode-linux-x64-stable.zip -d /usr/local
RUN tar xf  VSCode-linux-x64-stable.zip -C /usr/local
